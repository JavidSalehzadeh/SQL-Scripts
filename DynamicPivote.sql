...
select ad.guid as ActionDetailGuid,
       ad.id as ActionDetailID,
       f.ID as FactorID,
       ft.LatinTitle as factortypetitle,
       A.Code as ActionCode,
       A.IssueDate,
       A.FiscalYear,
       AT.ID as ActionTypeID,
       isnull(pt.id, 0) as PaymentTypeID,
       TotalAmount,
       ReturnAmount
into #Temp
from bdg_Actiondetails ad
    join bdg_Actiondetailfactors adf
        on ad.id = adf.ActionDetailID
...
(
    SELECT DISTINCT
        factortypetitle
    FROM #Temp
    where factortypetitle is not null
) AS tmp
--Get distinct values of the PIVOT Column with isnull
SELECT @PivotSelectColumnNames
    = ISNULL(@PivotSelectColumnNames + ',', '') + 'ISNULL(' + QUOTENAME(factortypetitle) + ', 0) AS '
      + QUOTENAME(factortypetitle)
FROM
(
    SELECT DISTINCT
        factortypetitle
...
SET @DynamicPivotQuery
    = N'SELECT ActionDetailGuid,ActionDetailID,ActionCode,IssueDate,FiscalYear,ActionTypeID,PaymentTypeID,TotalAmount,ReturnAmount, '
